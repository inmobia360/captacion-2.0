...
<label class="block"><span class="block text-xs font-bold text-slate-500 mb-1">Teléfono móvil (Opcional)</span><input id="professional-register-phone" type="tel" pattern="[0-9]{9,15}" autocomplete="tel" placeholder="Ej: 600123456" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm" /></label>
    ...
    function handleVeraImportPortalUrl() {
      ...
      showToast('Analizando inmueble con Vera IA y aplicando Datos Ciegos...', 'info');
      setTimeout(() => {
            ...
            <div class="flex flex-col md:flex-row">
              ...
              <div class="flex-1 p-4 space-y-3">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                  ...
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-light text-green shrink-0 w-fit">Perfil profesional</span>
                </div>
            ...
            <div>
              ...
              <div class="p-4 space-y-3">
                <div class="flex items-start justify-between gap-3">
                  ...
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-light text-green shrink-0">Perfil profesional</span>
                </div>
...
async function loadAndRenderCreditsLedger() {
    ...
    function getProfessionalProfileProgress(profile = {}) {
      ...
      const totalWeight = fields.reduce((sum, key) => sum + Number(PROFESSIONAL_PROFILE_WEIGHTS[key] || 0), 0) || 100;
      const earnedWeight = fields.reduce((sum, key) => sum + (String(effective[key] || '').trim() ? Number(PROFESSIONAL_PROFILE_WEIGHTS[key] || 0) : 0), 0);
      const filled = fields.filter(key => String(effective[key] || '').trim()).length;
      return { effective, completed:filled, total:fields.length, earnedWeight, totalWeight, percentage:Math.min(100, Math.round((earnedWeight / totalWeight) * 100)) };
    }
    ...
    async function importXmlFileFromInput(inputId, resultId) {
      ...
      try {
        ...
        if (data.ok) {
          ...
          if (resultDiv) resultDiv.innerHTML = `<div class="p-3 rounded-xl border border-green/20 bg-green-light text-xs text-slate-700"><strong class="text-green">Importación finalizada.</strong> ${imported} importadas, ${updated} actualizadas, ${pending} pendientes de revisión, ${failed} con error. Las propiedades quedan privadas hasta que pulses <strong>Publicar en Marketplace</strong>.${pending && fileFeedId ? ` <button type="button" onclick="showFeedPendingReviewPanel('${escapeHTML(fileFeedId)}')" class="ml-2 text-blue font-black underline">Revisar propiedades pendientes</button>` : ''}</div>`;
          showToast(pending ? `Archivo importado. ${pending} propiedades necesitan revisión.` : `Archivo importado: ${imported} propiedades. Revísalas y publícalas cuando quieras.`, pending ? 'info' : 'success');
          await loadXmlFeeds();
          await loadWordPressRealEstateRecords();
        } else {
